...
while [[ $# > 0 ]];do
    ...
    case $key in
        ...
        -k|--keep)
        INSTALL_WAY=1
        colorEcho ${BLUE} "ok configuracion update\n"
        ;;
        --zh)
        CHINESE=1
        colorEcho ${BLUE} "hola..\n"
...
help(){
    ...
    echo "  -k, --keep           keep the config.json  update"
    echo "      --remove         desinstalar v2ray,dj10go && vip7"
...
checkSys() {
    #检查是否为Root
    [ $(id -u) != "0" ] && { colorEcho ${RED} "Error: ingresar en modo root"; exit 1; }
    ...
    if [[ `command -v apt-get` ]];then
        ...
        colorEcho $RED "Sistema operativo no soprtado"
        exit 1
    fi
}
...
installFinish() {
    ...
    colorEcho  ${GREEN} "multi-v2ray ${WAY} Por Fin!!!\n"
    ...
    if [[ ${INSTALL_WAY} == 0 ]]; then
        ...
        echo -e "Escribir 'v2ray' para ingresar al MENU vip7 v2ray\n"
    fi
}
